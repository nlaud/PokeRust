...
- MAKE THE FRONTEND YIPPEE
  - [x] Simulate, Teams, and Formats pages (React + Vite + Tailwind in `frontend/`,
        Axum API server in `poke_rust/src/bin/server/` — see `frontend/README.md`)
  ...
  - [x] Engine: sampling-mode turn resolver (`simulator::sample_turn`) — walks one
        weighted trajectory instead of enumerating the outcome tree; the server uses
        it for interactive play, so all formats run at full 16 damage rolls.
